...
//          #'
//          #' @description constants previously computed in order to the M scale value be consistent
...
NumericVector normal_consistency_constants(int p) {
  NumericVector vaux = {0.404629,0.6944748,0.8985921,1.063144,1.204321,1.329791,
...
// [[Rcpp::export]]
double Mscale_pablo(NumericVector u, double b, double cc) {  // TODO: add default value b=0.5
  //            #' Mscale
  ...
  //            #' Robust statistics: theory and methods (with R). Wiley.
  //            #' @export
  ProfilerStart("profileo2");
  double sn = my_median(abs(u)) / 0.6745;
  if (sn == 0) return sn;

  NumericVector var;
  double quantity = mean(rhoOpt_pablo(var, cc)) - b; // indicates if the root is on the right
  if (quantity == 0) return(sn);
  
  while (quantity > 0) {
    sn = 1.5 * sn;
    quantity = mean(rhoOpt_pablo(u/sn, cc)) - b;
  }
  ...
  while ((i < 1000) & (err > 1e-10)) {
    var = u / sn;
    AA = mean(rhoOpt_pablo(var, cc));
    BB = mean(psiOpt_pablo(var, cc) * var);
...
// [[Rcpp::export]]
double Mscale_pablo_sin(NumericVector u, double b, double cc) {  // TODO: add default value b=0.5
  //            #' Mscale
  //            #' the M scale of an univariate sample (see reference below)
  //            #'
  //            #' @param u an univariate sample of size n.
