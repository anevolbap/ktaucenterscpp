...
ktaucenters_aux <- function(X, K, centers, tolmin, NiterMax) {
    ...
    while ((niter < NiterMax) & (tol > tolmin)) {
        for (h in 1:K) {
            ...
            # for (iw in 1:n){ repeatedCentersMatrix[iw, ] <- centers[h,] } a cada fila de x le resto el centerside y despues calculo
            # la distancia resultado: una matriz donde en cada fila tiene di1 di2 diK, donde dij es la distancia entre xi y el
            # centerside muj OLD distances[, h] <- sqrt((X - repeatedCentersMatrix)^2 %*% rep(1,p))
        ...
        # cluster is an array of nx1, that contains integers from 1 to K cluster[l] = j means that observation x_l is assigned to
        # cluster j.
        ...
        # Atenttion: when di=0. psi_1(dnor)=0 y psi_2(dnor)=0.  Then the weight w is undefined due to dividing by zero.  Given
        # that psi_1(0)=0, Wni can be obtained throug the derivative of psi_1 in this case. that is that the following lines do.
        ...
        for (jota in 1:K) {
            ...
            if ((sum(Wni[cluster == jota])) == 0) {
                # esto significa que el algoritmo converjio.  entonces no actualizo las X's.  pero si llega a pasar que son cero las
                # actualizo:
        ...
        # each X row is multiplied by their corresponding weight OLD OLD OLD OLD OLD OLDOLD OLD OLDOLD OLD OLD for (fila in 1:n){
        ...
        for (jota in 1:K) {
            ...
            if (auxx[jota] > 0) {
                # The 'as.matrix' in the line below is nedeed when the matrix has a single observation.  In that case R transforms it
        ...
        if (sum(auxx > 0) != K) {
            # if not all clusters are filled, ther are replaced for the furthest centers this is speccially important when the number
            # of clusters K is high.
    ...
    ret = list(tauPath = tauPath, niter = niter, centers = centers, cluster = cluster, emptyCluster = emptyCluster, tol = tol, 
        weights = weights, di = distances_min, Wni = Wni, emptyClusterFlag = emptyClusterFlag)
    return(ret)
}
