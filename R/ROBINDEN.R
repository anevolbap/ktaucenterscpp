...
#'
#' @description \code{ROBINDEN} searches for k initial cluster seeds
...
#' @export
#'
#' @details The centers are the observations located in the most dense
...
ROBINDEN <- function(D, data, k, mp = 10) {
  # THIS FUNCTION DO THE SAME THAN ROBIN BUT TAKing INTO ACCOUNT THE
  ...
  critRobin = sort(idp, decreasing = FALSE)[floor(vvv * n)]; # vvv=0.5
                                                             # implies
                                                             # critRobin=
                                                             # median(idp).
